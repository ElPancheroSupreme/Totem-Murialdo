// buffet.js - Versión específica para Buffet (id_punto_venta = 1)
...
function renderizarCategorias() {
    ...
    categorias.forEach(cat => {
        ...
        // Usar el icono de la base de datos en lugar de construir por nombre
        const iconoSrc = cat.icono ? `../assets/images/Iconos/${cat.icono}` : '../assets/images/Iconos/Icono_Snacks.svg';
        btn.innerHTML = `<img src="${iconoSrc}" alt="${cat.nombre}" onerror="this.src='../assets/images/Iconos/Icono_Snacks.svg'"><p>${cat.nombre}</p>`;
        btn.onclick = () => {
...
function mostrarCategoria(nombre) {
    ...
    if (productos.length === 0) {
        ...
        productos.forEach(prod => {
            ...
            const imagenUrl = prod.imagen || `Images/Buffet/${nombre}/${prod.id || prod.nombre}.png`;
            div.innerHTML = `
...
function agregarAlCarrito(nombre, precio, elemento) {
    ...
    if (existente) {
        ...
        carrito.push({
            ...
            imagen: prod.imagen || `Images/Buffet/${categoriaActual}/${prod.id || prod.nombre}.png`,
...
function mostrarOverlayPersonalizacion(prod) {
    ...
    overlay.querySelector('.OP_ImagenProducto').src = prod.imagen || `../assets/images/Buffet/${categoriaActual}/${prod.id || prod.nombre}.png`;
