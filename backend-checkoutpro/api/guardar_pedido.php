...
try {
    ...
    // Datos de reserva
    $es_reserva = intval($input['es_reserva'] ?? 0);
    // Solo asignar id_horario si es_reserva = 1
    $id_horario = ($es_reserva === 1 && !empty($input['id_horario'])) ? intval($input['id_horario']) : null;
    ...
    $stmt->execute([$numero_pedido, $id_punto_venta, $metodo_pago, $monto_total, $estado, $es_reserva, $id_horario]);
