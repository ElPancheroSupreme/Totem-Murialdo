...
try {
    if ($action === 'listar') {
        ...
        if ($mostrar_todos) {
            $stmt = $pdo->query('SELECT p.*, c.nombre as categoria_nombre, pv.nombre as punto_venta_nombre FROM productos p LEFT JOIN categorias c ON p.id_categoria = c.id_categoria LEFT JOIN puntos_venta pv ON p.id_punto_venta = pv.id_punto_venta ORDER BY p.estado DESC, p.id_producto DESC');
        } else {
            $stmt = $pdo->query('SELECT p.*, c.nombre as categoria_nombre, pv.nombre as punto_venta_nombre FROM productos p LEFT JOIN categorias c ON p.id_categoria = c.id_categoria LEFT JOIN puntos_venta pv ON p.id_punto_venta = pv.id_punto_venta WHERE p.estado = 1 ORDER BY p.id_producto DESC');
        }
