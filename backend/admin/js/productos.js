...
class ProductosManager {
    ...
    async cargarProveedores() {
        try {
            ...
            this.mostrarAlerta('Error al cargar proveedores', 'error');
        }
    }
    ...
    renderizarProductosLista(productos) {
        ...
        productos.forEach(producto => {
            ...
            const puntoVentaNombre = producto.punto_venta_nombre || 'Sin punto de venta';
            let puntoVentaBadge = '';
            if (puntoVentaNombre === 'Buffet') {
                puntoVentaBadge = '<span style="background-color: #10b98122; color: #10b981; padding: 4px 12px; border-radius: 12px; font-size: 0.875rem; font-weight: 500;">Buffet</span>';
            } else if (puntoVentaNombre === 'Kiosco') {
                puntoVentaBadge = '<span style="background-color: #3b82f622; color: #3b82f6; padding: 4px 12px; border-radius: 12px; font-size: 0.875rem; font-weight: 500;">Kiosco</span>';
            } else if (puntoVentaNombre === 'Ambos') {
                puntoVentaBadge = '<span style="background-color: #9c27b022; color: #9c27b0; padding: 4px 12px; border-radius: 12px; font-size: 0.875rem; font-weight: 500;">Ambos</span>';
            } else {
                puntoVentaBadge = `<span class="prod-nombre-desc">${puntoVentaNombre}</span>`;
            }
    ...
    renderizarProductosGrilla(productos) {
        ...
        productos.forEach(producto => {
            ...
            // Determinar color del badge según punto de venta para grilla
            const puntoVentaNombre = producto.punto_venta_nombre || 'Sin punto de venta';
            let puntoVentaBadge = '';
            if (puntoVentaNombre === 'Buffet') {
                puntoVentaBadge = '<span style="background-color: #10b98122; color: #10b981; padding: 2px 8px; border-radius: 8px; font-size: 0.75rem; font-weight: 500;">Buffet</span>';
            } else if (puntoVentaNombre === 'Kiosco') {
                puntoVentaBadge = '<span style="background-color: #3b82f622; color: #3b82f6; padding: 2px 8px; border-radius: 8px; font-size: 0.75rem; font-weight: 500;">Kiosco</span>';
            } else if (puntoVentaNombre === 'Ambos') {
                puntoVentaBadge = '<span style="background-color: #9c27b022; color: #9c27b0; padding: 2px 8px; border-radius: 8px; font-size: 0.75rem; font-weight: 500;">Ambos</span>';
            } else {
                puntoVentaBadge = `<span style="font-size: 0.75rem; color: #6b7280;">${puntoVentaNombre}</span>`;
            }
            ...
            html += `
                <div class="${cardClass}">
                    ...
                    <div class="producto-info">
                        ...
                        <p class="producto-categoria">${producto.categoria_nombre || 'Sin categoría'} • ${puntoVentaBadge}</p>
    ...
    actualizarSelectProveedores() {
        const filter = document.getElementById('proveedor-filter');
    ...
    abrirModal(tipo) {
        ...
        if (tipo === 'crear') {
            ...
            // Para nuevos productos, limpiar categorías hasta que se seleccione punto de venta
            this.actualizarSelectCategorias();
        } else if (tipo === 'editar') {
    ...
    setupFiltros() {
        ...
        const filtros = ['categoria-filter', 'punto-venta-filter', 'proveedor-filter', 'estado-filter', 'orden-filter'];
        filtros.forEach(filtroId => {
            ...
            if (filtroElement) {
                filtroElement.addEventListener('change', (e) => {
                    ...
                    } else if (filtroId === 'proveedor-filter') {
    ...
    async aplicarFiltros() {
        ...
        if (this.filtros.proveedor) {
            productosFiltrados = productosFiltrados.filter(producto =>
                producto.id_proveedor == this.filtros.proveedor
            );
        }
